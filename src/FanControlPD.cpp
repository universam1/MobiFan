...
void FanControlPD::setPowerPercent(float pct) {
...
void FanControlPD::applyCfg(float volts) {
  if (volts <= 5.0f) {
    digitalWrite(PIN_PD_CFG1, HIGH);
    // CFG2/3 don't matter for 5V but set them HIGH for a defined state
    digitalWrite(PIN_PD_CFG2, HIGH);
    digitalWrite(PIN_PD_CFG3, HIGH);
  } else if (volts <= 9.0f) {
